...
export default function Home() {
  ...
  const [printOrientation, setPrintOrientation] = useState<{ weekly: "portrait" | "landscape"; daily: "portrait" | "landscape"; summary: "portrait" | "landscape" }>({ weekly: "landscape", daily: "portrait", summary: "portrait" });
  ...
  function printPlan(mode: "weekly" | "daily" | "summary") {
    ...
    const size = printOrientation[mode] === "landscape" ? "11in 8.5in" : "8.5in 11in";
    styleEl.textContent = `@page{size:${size};margin:.4in}`;
    window.setTimeout(() => window.print(), 80);
  }
  function setPdfOrientation(mode: "weekly" | "daily" | "summary", value: "portrait" | "landscape") {
    setPrintOrientation((current) => ({ ...current, [mode]: value }));
  }
  ...
  return <main className={`planner-app view-${viewMode} mode-${displayMode ? "display" : "edit"} ${presenting ? "projection-mode" : ""} print-${printMode}`}>
    ...
    <section className="control-deck screen-only" aria-label="Planner controls">
      ...
      <div className="control-actions">
        ...
        <details className="more-menu"><summary>More</summary><div>
          ...
          <label>Weekly PDF orientation<div className="orientation-picker" role="group" aria-label="Weekly PDF orientation"><button type="button" className={printOrientation.weekly === "portrait" ? "active" : ""} aria-pressed={printOrientation.weekly === "portrait"} onClick={() => setPdfOrientation("weekly", "portrait")}>Portrait</button><button type="button" className={printOrientation.weekly === "landscape" ? "active" : ""} aria-pressed={printOrientation.weekly === "landscape"} onClick={() => setPdfOrientation("weekly", "landscape")}>Landscape</button></div></label>
          ...
          <label>Daily PDF orientation<div className="orientation-picker" role="group" aria-label="Daily PDF orientation"><button type="button" className={printOrientation.daily === "portrait" ? "active" : ""} aria-pressed={printOrientation.daily === "portrait"} onClick={() => setPdfOrientation("daily", "portrait")}>Portrait</button><button type="button" className={printOrientation.daily === "landscape" ? "active" : ""} aria-pressed={printOrientation.daily === "landscape"} onClick={() => setPdfOrientation("daily", "landscape")}>Landscape</button></div></label>
    ...
    {briefOpen && <div className="modal-backdrop weekly-summary-backdrop screen-only" onMouseDown={() => setBriefOpen(false)}><section className="drawer weekly-drawer" role="dialog" aria-modal="true" aria-labelledby="brief-title" onMouseDown={(event) => event.stopPropagation()}><div className="drawer-head"><div><p className="eyebrow">Generated from the current plan</p><h2 id="brief-title">Weekly summary</h2></div><button type="button" onClick={() => setBriefOpen(false)} aria-label="Close">×</button></div>
      ...
      <details className="brief-editor"><summary><span><strong>Edit optional week-level context</strong><small>Topic, central question, certification notes, lab context, and other details. None are required for the report above.</small></span><b>＋</b></summary><div className="field-grid">{metaFields.map(([key, label, placeholder]) => <label key={key} className={key === "topic" || key === "centralQuestion" ? "wide" : ""}><span>{label}</span><textarea rows={key === "topic" ? 2 : 3} value={planner.meta[key]} placeholder={placeholder} onChange={(event) => updateMeta(key, event.target.value)} /></label>)}</div></details><div className="drawer-actions"><div className="orientation-picker drawer-orientation" role="group" aria-label="Weekly summary PDF orientation"><button type="button" className={printOrientation.summary === "portrait" ? "active" : ""} aria-pressed={printOrientation.summary === "portrait"} onClick={() => setPdfOrientation("summary", "portrait")}>Portrait</button><button type="button" className={printOrientation.summary === "landscape" ? "active" : ""} aria-pressed={printOrientation.summary === "landscape"} onClick={() => setPdfOrientation("summary", "landscape")}>Landscape</button></div><button type="button" onClick={() => printPlan("summary")}>Print / save PDF</button><button className="primary" type="button" onClick={() => setBriefOpen(false)}>Done</button></div></section></div>}
